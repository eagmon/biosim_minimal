...
class E2eMigrator extends project_migrator_1.ProjectMigrator {
    ...
    updateCypress10ConfigFile(configFilePath) {
        ...
        let sourceFile = tsquery_1.tsquery.ast(fileContent);
        const recorder = new utilities_1.FileChangeRecorder(this.tree, configFilePath);
        const defineConfigExpression = tsquery_1.tsquery.query(sourceFile, 'CallExpression:has(Identifier[name=defineConfig]) > ObjectLiteralExpression')[0];
        ...
        defineConfigExpression.forEachChild((node) => {
            if ((0, typescript_2.isPropertyAssignment)(node) && node.name.getText() === 'component') {
                componentNode = node;
                return;
            }
            if ((0, typescript_2.isPropertyAssignment)(node) && node.name.getText() === 'e2e') {
                e2eNode = node;
                return;
            }
            if ((0, typescript_2.isPropertyAssignment)(node)) {
    ...
    updateCypressComponentConfig(componentNode, recorder) {
        ...
        if (!(0, typescript_2.isObjectLiteralExpression)(componentNode.initializer)) {
    ...
    updateCypressE2EConfig(configFilePath, defineConfigNode, e2eNode, recorder, _a) {
        ...
        ${Object.entries(globalConfig)
            ...
            if (!(0, typescript_2.isObjectLiteralExpression)(e2eNode.initializer)) {
        ...
        recorder.applyChanges();
        const sourceFile = tsquery_1.tsquery.ast(recorder.content);
        (0, ast_utils_1.insertImport)(this.tree, sourceFile, configFilePath, 'nxE2EPreset', '@nrwl/cypress/plugins/cypress-preset');
    ...
    isValidPathLikePropertyWithStringLiteralValue(node, properties) {
        ...
        if (node.initializer.kind === typescript_2.SyntaxKind.UndefinedKeyword ||
            node.initializer.kind === typescript_2.SyntaxKind.NullKeyword ||
            node.initializer.kind === typescript_2.SyntaxKind.FalseKeyword) {
            return false;
        }
        if (!(0, typescript_2.isStringLiteralLike)(node.initializer)) {
            if ((0, typescript_2.isTemplateExpression)(node.initializer)) {
