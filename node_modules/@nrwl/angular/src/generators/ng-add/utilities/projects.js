"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.convertAllToNxProjects = exports.convertToNxProject = exports.getAllProjects = void 0;
const devkit_1 = require("@nrwl/devkit");
const angular_json_1 = require("nx/src/adapter/angular-json");
function getAllProjects(tree) {
    const projects = (0, devkit_1.getProjects)(tree);
    const result = {
        apps: [],
        libs: [],
    };
    for (const [name, project] of projects) {
        const migrationProject = {
            config: project,
            name,
        };
        if (project.projectType === 'library') {
            result.libs.push(migrationProject);
        }
        else {
            result.apps.push(migrationProject);
        }
    }
    return result;
}
exports.getAllProjects = getAllProjects;
function convertToNxProject(tree, projectName) {
    const angularJson = (0, devkit_1.readJson)(tree, 'angular.json');
    const project = (0, angular_json_1.toNewFormat)(angularJson).projects[projectName];
    project.name = projectName;
    (0, devkit_1.addProjectConfiguration)(tree, projectName, project);
    delete angularJson.projects[projectName];
    (0, devkit_1.writeJson)(tree, 'angular.json', Object.assign(Object.assign({}, angularJson), { version: 1 }));
}
exports.convertToNxProject = convertToNxProject;
function convertAllToNxProjects(tree) {
    const angularJson = (0, devkit_1.readJson)(tree, 'angular.json');
    const projects = (0, angular_json_1.toNewFormat)(angularJson).projects;
    for (const projectName of Object.keys(projects)) {
        projects[projectName].name = projectName;
        (0, devkit_1.addProjectConfiguration)(tree, projectName, projects[projectName]);
        delete angularJson.projects[projectName];
    }
    (0, devkit_1.writeJson)(tree, 'angular.json', Object.assign(Object.assign({}, angularJson), { version: 1 }));
}
exports.convertAllToNxProjects = convertAllToNxProjects;
//# sourceMappingURL=projects.js.map