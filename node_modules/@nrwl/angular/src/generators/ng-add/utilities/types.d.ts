import type { ProjectConfiguration } from '@nrwl/devkit';
export declare type MigrationProjectConfiguration = {
    config: ProjectConfiguration;
    name: string;
};
export declare type WorkspaceProjects = {
    apps: MigrationProjectConfiguration[];
    libs: MigrationProjectConfiguration[];
};
export declare type ProjectMigrationInfo = {
    name: string;
    oldRoot: string;
    oldSourceRoot: string;
    newRoot: string;
    newSourceRoot: string;
};
export declare type WorkspaceRootFileType = 'karma' | 'eslint';
export declare type WorkspaceRootFileTypesInfo = Record<WorkspaceRootFileType, boolean>;
export declare type ValidationError = {
    message?: string;
    messageGroup?: {
        title: string;
        messages: string[];
    };
    hint?: string;
};
export declare type ValidationResult = ValidationError[] | null;
export declare type Target = {
    builders: string[];
    acceptMultipleDefinitions?: boolean;
};
