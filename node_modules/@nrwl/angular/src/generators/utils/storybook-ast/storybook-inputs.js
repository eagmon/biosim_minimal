...
const typescript_2 = require("@nrwl/workspace/src/utilities/typescript");
const typescript_3 = require("typescript");
const ast_utils_1 = require("../../../utils/nx-devkit/ast-utils");
function getInputPropertyDeclarations(tree, path) {
    const file = (0, ast_utils_1.getTsSourceFile)(tree, path);
    const decorators = (0, typescript_2.getSourceNodes)(file).filter((node) => node.kind === typescript_3.SyntaxKind.Decorator);
    return decorators
        .filter((decorator) => (0, typescript_1.findNodes)(decorator, typescript_3.SyntaxKind.Identifier).some((node) => node.getText() === 'Input'))
...
function getComponentProps(tree, componentPath, getArgsDefaultValueFn = getArgsDefaultValue, useDecoratorName = true) {
    const props = getInputPropertyDeclarations(tree, componentPath).map((node) => {
        const decoratorContent = (0, typescript_1.findNodes)((0, typescript_1.findNodes)(node, typescript_3.SyntaxKind.Decorator).find((n) => n.getText().startsWith('@Input')), typescript_3.SyntaxKind.StringLiteral);
...
function getKnobType(property) {
    ...
    if (property.initializer) {
        const initializerKindToKnobType = {
            [typescript_3.SyntaxKind.StringLiteral]: 'text',
            [typescript_3.SyntaxKind.NumericLiteral]: 'number',
            [typescript_3.SyntaxKind.TrueKeyword]: 'boolean',
            [typescript_3.SyntaxKind.FalseKeyword]: 'boolean',
        };
...
//# sourceMappingURL=storybook-inputs.js.map