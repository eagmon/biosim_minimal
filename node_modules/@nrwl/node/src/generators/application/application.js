...
const shared = require("@nrwl/workspace/src/utils/create-ts-config");
...
function getWebpackBuildConfig(project, options) {
    return {
        ...
        configurations: {
            production: Object.assign(Object.assign({}, (options.docker && { generateLockfile: true })), { optimization: true, extractLicenses: true, inspect: false }),
        },
    };
}
function getEsBuildConfig(project, options) {
    return {
        ...
        configurations: {
            production: Object.assign(Object.assign({}, (options.docker && { generateLockfile: true })), { esbuildOptions: { sourcemap: false } }),
        },
    };
}
...
function addAppFiles(tree, options) {
    (0, devkit_1.generateFiles)(tree, (0, path_1.join)(__dirname, './files/common'), options.appProjectRoot, Object.assign(Object.assign({}, options), { tmpl: '', name: options.name, root: options.appProjectRoot, offset: (0, devkit_1.offsetFromRoot)(options.appProjectRoot), rootTsConfigPath: (0, typescript_1.getRelativePathToRootTsConfig)(tree, options.appProjectRoot) }));
...
function addProjectDependencies(tree, options) {
    ...
    return (0, devkit_1.addDependenciesToPackageJson)(tree, {}, Object.assign(Object.assign({}, frameworkDependencies[options.framework]), bundlers[options.bundler]));
}
function updateTsConfigOptions(tree, options) {
    (0, devkit_1.updateJson)(tree, `${options.appProjectRoot}/tsconfig.json`, (json) => {
        if (options.rootProject) {
            return Object.assign(Object.assign({ compilerOptions: Object.assign(Object.assign(Object.assign({}, shared.tsConfigBaseOptions), json.compilerOptions), { esModuleInterop: true }) }, json), { extends: undefined, exclude: ['node_modules', 'tmp'] });
        }
        else {
...
function applicationGenerator(tree, schema) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        if (options.e2eTestRunner === 'jest') {
            const e2eTask = yield (0, e2e_project_1.e2eProjectGenerator)(tree, Object.assign(Object.assign({}, options), { projectType: options.framework === 'none' ? 'cli' : 'server', name: options.rootProject ? 'e2e' : `${options.name}-e2e`, project: options.name, port: options.port }));
            tasks.push(e2eTask);
        }
        if (options.js) {
        ...
        if (options.docker) {
            const dockerTask = yield (0, setup_docker_1.setupDockerGenerator)(tree, Object.assign(Object.assign({}, options), { project: options.name }));
